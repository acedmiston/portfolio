...
function areValuesEqual(a, b) {
  ...
  if (Array.isArray(a) && Array.isArray(b)) {
    ...
    return a.every(function (item, index) {
      return areValuesEqual(item, b[index]);
    });
  }
      ...
      try {
        ...
        console.log(
          chalk_1.default.yellow(
            'No existing translations found for '.concat(locale)
          )
        );
      }
      return [2 /*return*/, {}];
        ...
        return __generator(this, function (_d) {
          switch (_d.label) {
            ...
            case 1:
              ...
              return [
                4 /*yield*/,
                processEntries(sourceValue, existing[key], fullPath),
              ];
            case 2:
              _d.sent();
              return [3 /*break*/, 4];
            case 3:
              ...
              if (!existingEntry || forceUpdate) {
                if (!existingEntry) {
                  newCount++;
                  console.log(
                    chalk_1.default.green('+ New: '.concat(fullPath))
                  );
                } else {
                  updatedCount++;
                  console.log(
                    chalk_1.default.magenta(
                      '\u21BB Force updated: '.concat(fullPath)
                    )
                  );
                }
                setNestedValue(
              ...
              if (isTranslationEntry(existingEntry)) {
                if (!areValuesEqual(existingEntry.sourceMessage, sourceValue)) {
                  ...
                  console.log(
                    chalk_1.default.yellow(
                      '\u21BB Updated (source changed): '.concat(fullPath)
                    )
                  );
                  setNestedValue(
                    pendingTranslations,
              ...
              console.log(
                chalk_1.default.blue(
                  '\uD83D\uDCDD Added source tracking: '.concat(fullPath)
                )
              );
              setNestedValue(
                pendingTranslations,
    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          ...
          sourcePath = path_1.default.join(
            __dirname,
            '..',
            'messages',
            ''.concat(sourceLanguage, '.json')
          );
        ...
        case 1:
          ...
          return [
            4 /*yield*/,
            processEntries(sourceJson, existingTranslations),
          ];
        case 2:
          _a.sent();
          if (Object.keys(pendingTranslations).length === 0) {
            ...
            console.log(
              '\n'
                ...
                .concat(
                  chalk_1.default.blue(
                    'Summary for '.concat(targetLanguage.toUpperCase())
                  ),
                  '\n'
                )
                .concat('-'.repeat(50))
            ...
            console.log(
              chalk_1.default.blue('\u2022 New translations: '.concat(newCount))
            );
            console.log(
              chalk_1.default.blue(
                '\u2022 Updated translations: '.concat(updatedCount)
              )
            );
            console.log(
              chalk_1.default.blue(
                '\u2022 Unchanged translations: '.concat(unchangedCount)
              )
            );
              ...
              messages: [
                {
                  role: 'system',
                  content:
                    'You are a professional translator. Translate the following JSON content from '
                      .concat(sourceLanguage, ' to ')
                      .concat(
                        targetLanguage,
                ...
                {
                  role: 'user',
                  content: 'Translate ONLY these JSON parts to '
                    .concat(targetLanguage, ':\n')
        ...
        case 3:
          ...
          try {
            ...
            preFilePath = path_1.default.join(
              PRE_FOLDER,
              ''.concat(targetLanguage, '.json')
            );
            ...
            console.log(
              chalk_1.default.green(
                '\u2705 Translation completed in '.concat(translationTime, 's')
              )
            );
            console.log(
              '\n'
            ...
            console.log(
              chalk_1.default.blue(
                '\u2022 Translation time: '.concat(translationTime, 's')
              )
            );
            console.log(''.concat('-'.repeat(50), '\n'));
          } catch (error) {
            console.error(
              chalk_1.default.red('Error processing translation:'),
              error
            );
          }
          return [2 /*return*/];
...
function mergeTranslations(translated, source, target, path) {
  ...
  for (var key in source) {
    var currentPath = __spreadArray(
      __spreadArray([], path, true),
      [key],
      false
    );
...
function compileCleanTranslation(withMetadata, locale) {
  ...
  var finalPath = path_1.default.join(
    __dirname,
    '..',
    'messages',
    ''.concat(locale, '.json')
  ...
  console.log(
    chalk_1.default.green(
      '\u2705 Clean translation file saved to '.concat(finalPath)
    )
  );
}
function createSourcePreFile(sourceLanguage) {
  console.log(
    chalk_1.default.blue(
      'Creating pre file for source language ('.concat(sourceLanguage, ')...')
    )
  );
  try {
    // Read source language file
    var sourcePath = path_1.default.join(
      __dirname,
      '..',
      'messages',
      ''.concat(sourceLanguage, '.json')
    ...
    // Save to pre folder
    var preFilePath = path_1.default.join(
      PRE_FOLDER,
      ''.concat(sourceLanguage, '.json')
    );
    ...
    console.log(
      chalk_1.default.green(
        '\u2705 Source pre file created at '.concat(preFilePath)
      )
    );
  } catch (error) {
    console.error(
      chalk_1.default.red('Error creating source pre file:'),
      error
    );
  }
}
    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          ...
          console.log(
            'No target language provided. Translating from '
              .concat(sourceLanguage, ' to all available languages: ')
              .concat(targetLanguages.join(', '))
          );
        ...
        case 2:
          ...
          return [
            4 /*yield*/,
            translateJson(sourceLanguage, lang, forceUpdate),
          ];
        case 3:
          _a.sent();
          return [3 /*break*/, 5];
        case 4:
          error_1 = _a.sent();
          console.error(
            chalk_1.default.red('Error processing '.concat(lang, ':')),
            error_1
          );
          return [3 /*break*/, 5];
        case 5:
